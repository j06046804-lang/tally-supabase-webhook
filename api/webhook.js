...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Verificamos que lleguen respuestas
    if (!body.form_response || !body.form_response.answers) {
      return res.status(400).json({ error: 'No se encontraron respuestas en el body' });
    }

    const answers = body.form_response.answers;

    // Mapeamos los datos de Tally a un objeto con las mismas columnas de la tabla
    const data = {};
    answers.forEach(answer => {
      const key = answer.field.ref; // El 'ref' debe coincidir con el nombre de la columna en la tabla
      if (answer.text) data[key] = answer.text;
      else if (answer.number !== undefined) data[key] = answer.number;
      else if (answer.choice) data[key] = answer.choice.label;
      else data[key] = null;
    });
