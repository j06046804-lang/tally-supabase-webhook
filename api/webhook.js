...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Mapeamos cada respuesta al nombre de columna (field.ref)
    answers.forEach(answer => {
      const key = answer.field.ref;
      switch (answer.type) {
        case 'text':
          data[key] = answer.text || null;
          break;
        case 'number':
          data[key] = answer.number !== undefined ? answer.number : null;
          break;
        case 'choice':
          data[key] = answer.choice?.label || null;
          break;
        case 'choices':
          data[key] = answer.choices?.labels?.join(', ') || null;
          break;
        case 'boolean':
          data[key] = answer.boolean !== undefined ? answer.boolean : null;
          break;
        default:
          data[key] = null;
      }
    ...
    // Insertamos en la tabla de Supabase con todos los campos
